...
from tkinter import simpledialog,messagebox,Label,Tk
import atbswp.settings as settings

from atbswp.custom_widgets import SliderDialog
...
class RecordCtrl:
    ...
    def action(self, event):
        ...
        if event.EventObject.Value:
            ...
            #     None, "Recording started!", "Alert", wx.ICON_INFORMATION)
            # dlg.ShowModal()
            # dlg.Destroy()
            self.recording = True
            ...
            dlg = wx.MessageDialog(
                    None, "Recoding saved!", "Alert", wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
